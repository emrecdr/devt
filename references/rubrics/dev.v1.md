...
- `coverage_complete: true` catches the silent-skip failure mode where a JSON-first tester loops over a truncated upstream `impl-summary.json::files_changed` and reports `status=DONE` while testing nothing. The tester computes this boolean by comparing its `coverage_files` to the upstream sidecar's `files_changed` (see `agents/tester.md`); `false` short-circuits to a tester re-dispatch with the missing files as `<review_feedback>`. Converts a previously-invisible truncation propagation into a hard process gate. Untestable files (type-only, config-only, generated) go in `coverage_exempt` with a reason and still count complete: this is a strict scalar equality with no allow-list, so `false` there would re-dispatch into the same answer forever.
