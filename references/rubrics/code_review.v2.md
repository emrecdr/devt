...
| ---- | --- | ------------ |
| **A. Scope coverage** | Every file listed in `.devt/state/code-review-input.md` appears at least once in `review.md` — either as an explicit finding OR an explicit "no issues found in `<file>`" line. | `grep` each scope file against `review.md`. A file with zero mentions is a gap. |
