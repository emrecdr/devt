...
| `failed`         | The review is structurally broken (status missing, no findings list, can't parse the markdown) OR code-review-input.md is missing entirely. | Workflow surfaces BLOCKED to the user; no further retry.                                                              |
...
| ---- | --- | ------------ |
| **A. Scope coverage** | Every file listed in `.devt/state/code-review-input.md` appears at least once in `review.md` — either as an explicit finding OR an explicit "no issues found in `<file>`" line. **This axis measures files MENTIONED, not files assigned to a lane.** A review claiming "N of N covered" on the strength of the partition, without N files appearing in the prose, fails this axis. Any N-of-M coverage figure must name which count it reports: `files_assigned`, `files_mentioned`, `files_with_findings`, or `files_cleared`. | `grep` each scope file against `review.md`. A file with zero mentions is a gap — including one a lane verified thoroughly, because a verification that didn't reach the review is indistinguishable from a file nobody read. |
  ...
  "revisions": [
    {
      "id": "A-1",
      "axis": "scope_coverage",
      "criterion": "Every file in code-review-input.md appears at least once in review.md",
      "gap": "src/auth/middleware.ts is in code-review-input.md but has zero mentions in review.md — reviewer skipped it",
...
- `.devt/state/code-review-input.md` is missing (the reviewer was dispatched without scope — a workflow bug, not a review quality issue).
...
```
              ...
              │
              ├─ code-review-input.md missing? ─────────────────────────────► failed
              │
