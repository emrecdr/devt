...
node bin/devt-tools.cjs memory init|index|query|get|affects|list|links|active|rejected-keywords|validate|supersede|suggest
...
- **Graphify-scan partition** — the `graphify_scan_prep` ACTIVE/SKIP/RECOVERY decision bodies live once in `workflows/graphify-scan.steps.md`, loaded at a `GRAPHIFY-STEP:decision` pointer by dev-workflow (MODE=dev), quick-implement (MODE=qi), and debug (MODE=debug); each parent keeps only its scan-prep bash resident. The copy-paste era drifted at the wording level across all three. `research-task.md` still carries an older bash-echo variant (migration pending). F13 enforces the partition.
