...
- **Two-layer memory model (v0.28.0+)**: devt persists structured knowledge across two distinct layers. (1) `.devt/state/` — per-workflow ephemeral artifacts (`decisions.md` DEC-xxx captures, `lessons.yaml` retro hand-off draft, plan/spec/research/etc); resets between workflows. (2) `.devt/memory/{decisions,concepts,flows,rejected,lessons}/*.md` — permanent canonical knowledge in 5 doc types: ADR (decisions), CON (concepts), FLOW (process), REJ (tombstones), LES (operational lessons). All share the same frontmatter shape (id/doc_type/status/confidence/summary/affects_paths/affects_symbols/links). FTS5-indexed in `.devt/memory/index.db` (gitignored, regenerable). The retro→curator pipeline writes `.devt/state/lessons.yaml` drafts → curator gates each via AskUserQuestion → on approval writes `LES-NNNN.md` to `.devt/memory/lessons/`. v0.17.0 (Phase 2) wires Graphify symbol anchoring + curator-gated promotion + claude-mem ⚖️/🔵 harvest + Graphify-first protocol across dev skills and workflows. v0.18.0 (Phase 3) ships the Topic Pre-Flight Brief auto-fired from 9 dev workflows + vendored read-only MCP query layer (`bin/devt-memory-mcp.cjs`, 10 helper tools + SELECT-only escape hatch) + Two-Tier Pre-Flight Protocol (Tier 1 Brief + Tier 2 PreToolUse hook in warn mode by default) + golden rules R14/R15 + auto-index PostToolUse hook. v0.19.0 (Phase 4) flips block-mode + wide-surface integration polish. v0.28.0 collapses the legacy 3-layer model (state + flat-file playbook + structured memory) into the current 2-layer model — operational lessons gain a 5th doc type and live alongside architectural docs. See `docs/MEMORY.md` for the full guide.
- **Post-v0.19 memory hardening (v0.20.0–v0.26.0)**: v0.20.0 ships `token-report` session telemetry + portable bundle export/import with `--prefix` remapping + post-commit-validate hook + Phase 5 smoke. v0.21.0 adds MCP tool-call telemetry to `_mcp-trace.jsonl` (privacy-safe — sizes + sha256:12 fingerprints, no raw args) + `mcp-stats` aggregator + comprehensive bundle round-trip smoke. v0.22.0 adds configurable multi-root memory paths (last-wins precedence, `source_root` provenance, explicit `conflicts[]` reporting). v0.23.0 promotes 4 native MEM_* checks to `bin/modules/health.cjs` (`MEM_PATH_UNREACHABLE`, `MEM_INDEX_STALE`, `MEM_VALIDATE_ERRORS`, `MEM_CONFLICT_HIGH`) + ships `memory paths [--validate]` and `memory diff <root-a> <root-b>` subcommands. v0.24.0 ships defense-in-depth `safeJsonParse` wrapping at every JSON parse boundary in `bin/` (22 sites; only remaining `JSON.parse` is inside the wrapper itself — auditable via single grep). v0.25.0 closes targeted gaps from CCA v21.0 §5/§10: SQLite `COLLATE NOCASE` for symbol lookups (so `affects_symbols: [UserService]` matches a query for `userService`), self-link detection in `memory validate`, and 4 SQL views (`pending_review`, `speculative_candidates`, `constraint_chains`, `stale_speculative` — all queryable through MCP `query_index` SELECT-only escape hatch). The `runSql` helper now strips SQL line comments before splitting on `;` (a semicolon in a comment used to silently abort schema init mid-stream). v0.26.0 ships (a) **stale-symbol detection in `memory validate`** via `graphify.queryGraph()` — closes the last gap from CCA v27 §2 "Tricky Parts: Symbol Decay" so renaming a class flags every doc still pointing at the old name (graceful skip when Graphify is disabled). Includes a **circuit breaker** (3 consecutive Graphify failures abort with a single `graphify-unreachable` warning) and a **correctness fix** so `degraded: true` results never produce false-positive `stale-symbol` flags during transient subprocess failures; (b) **unconditional `harvest_observations` step** in dev-workflow / lesson-extraction / quick-implement — decouples cheap claude-mem ⚖️/🔵 harvest from gated curator review so observations from quick/SIMPLE workflows are buffered into `_suggestions.md` even when retro+curator is skipped; (c) **dual-path curator dispatch** (playbook + memory layer) replacing the previously playbook-only `<files_to_read>`; (d) **graph-staleness alert in Pre-Flight Brief** when `graphify.freshness().lag_commits >= 10` (the `lag_commits` field is also newly delivered — JSDoc had promised it since v0.17.0 but the impl never computed it). `freshness()` itself is now a fast-path probe — reads only the first 8KB of `graph.json` and regex-extracts `built_at_commit` instead of full-parsing under a 200MB cap; (e) **`GRAPHIFY_MCP_UNREGISTERED` health check** (info-severity, warn-only — never auto-edits `.mcp.json`). The check reads `.mcp.json` first and only spawns `graphify --help` when registration is missing, removing a 50–200ms subprocess from every `/devt:health` invocation in the common case; (f) eliminated dynamic-RegExp construction in 2 hot paths (`graphify.cjs::blastRadius` god-node detection rewritten as `indexOf`-walk with charCode boundary checks; `memory.cjs::matchesPattern` glob check rewritten as recursive descent `globMatch`, with `**/**/...` repeats collapsed pre-match to bound the recursive branching factor). The glob rewrite incidentally fixed a pre-existing bug where `src/**` failed to match `src/foo/bar.ts` because the regex builder's escape-then-replace order corrupted the `**` substitution. Known limit: `validateSymbolsViaGraphify` is still sync — projects with 100+ unique symbols can take 5–20s on `memory validate`; parallelizing requires sync→async cascade through `health.cjs::runChecks` + CLI dispatcher, deferred until users hit the wall.
- **Two-Tier Pre-Flight Protocol (v0.18.0+)**: Tier 1 = workflow auto-fires `/devt:preflight "<task>"` to write `.devt/state/preflight-brief.md` listing governing ADRs/Concepts/Flows + REJ tombstones + lessons + (with Graphify) blast radius. All 8 dev agents preload `devt:memory-pre-flight` and read the Brief first. As of v0.27.0, 7 of the 8 also preload a domain skill that routes through the canonical Graphify-first protocol (`graphify-helpers` wrapper with grep fallback): `programmer`/`debugger`/`researcher` → `codebase-scan`, `code-reviewer` → `code-review-guide`, `verifier` → `verification-patterns`, `tester` → `tdd-patterns`, `architect` → `graphify-helpers` directly. Only `docs-writer` keeps memory-pre-flight alone (no code search). Tier 2 = `hooks/pre-flight-guard.sh` (PreToolUse on Edit/Write/NotebookEdit) checks `.devt/state/scratchpad.md` for a `PREFLIGHT <ts> edit <path> :: <governing IDs>` line. `memory.preflight_mode`: `off` no-op | `warn` advisory | `block` denies the edit (default in v0.19.0+). The PostToolUse `hooks/memory-auto-index.sh` rebuilds the FTS5 index whenever `.devt/memory/**.md` is touched. `/devt:init` now prompts via AskUserQuestion to run `graphify hook install` when Graphify is on PATH but its post-commit hook isn't registered — keeps the graph cache fresh so blast-radius and stale-symbol checks stay accurate. Override per-project in `.devt/config.json` if your team needs a slower onboarding ramp. See `docs/MEMORY.md` for the full guide.
