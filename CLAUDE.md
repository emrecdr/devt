...
- **Graphify-scan partition** — the `graphify_scan_prep` ACTIVE/SKIP/RECOVERY decision bodies live once in `workflows/graphify-scan.steps.md`, loaded at a `GRAPHIFY-STEP:decision` pointer by dev-workflow (MODE=dev), quick-implement (MODE=qi), debug (MODE=debug), and research-task (MODE=research); each parent keeps only its `preflight scan-prep` bash resident. The copy-paste era drifted at the wording level across all four. F13 enforces the partition.
