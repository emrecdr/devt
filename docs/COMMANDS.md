...
One-time setup for a new project. Scaffolds `.dev-rules/` with project conventions and creates `.devt.json`.
...
3. Copies template files to `.dev-rules/`
4. Creates `.devt.json`, `.devt-state/`, `learning-playbook.md`
...
| `/devt:autoskill` | Propose plugin improvements | Workflow autoskill step (STANDARD + COMPLEX) |
...
```
specify     --> spec.md          --> plan, workflow (requirements source)
research    --> research.md      --> plan, programmer (approach guidance)
clarify     --> decisions.md     --> programmer, verifier (decision IDs: DEC-001, DEC-002)
plan        --> plan.md          --> programmer, verifier (task breakdown)
programmer  --> impl-summary.md  --> tester, reviewer, verifier, docs-writer
tester      --> test-summary.md  --> reviewer, verifier, docs-writer
reviewer    --> review.md        --> programmer (if NEEDS_WORK), verifier
verifier    --> verification.md  --> programmer (if GAPS_FOUND)
architect   --> arch-review.md   --> programmer
retro       --> lessons.yaml     --> curator
curator     --> learning-playbook.md --> future workflows
