...
| `lane-files/` | `state register-lane` + `register-lanes` (round 8 Tier C) | directory holding `<lane-id>.json` per-lane files sidecars (each: `{id, community, files[], registered_at, repo_root, base_ref, size_class, size_basis, est_loc, diff_artifact}`); written because the lane-files array can't safely round-trip through `serializeSimpleYaml`'s primitive-only lane field encoder. A `register-lanes` batch DECLARES the full partition and prunes ids outside it from the registry + this dir + `lane-diff-<id>.txt` together (loud `pruned_stale` telemetry) — same-id overwrite alone left every id above a smaller partition's count orphaned across runs (field: L6-L8 sidecars from two partitions back survived on disk; `reset-soft` never descends into this dir). Single `register-lane` calls still merge. | ✗ (archived with hard reset — lane registrations are scope-bound to their workflow, same rationale as `lane-diff-L*.txt`; `state cleanup` never bulk-archives it mid-workflow via the audit's canonical-subdir list) |
...
| `gate-trace.jsonl` | `state` gate persistence (`state-gates.cjs`) | JSONL, one record per gate decision. Survives reset for the same reason `dispatch-warnings.jsonl` does — and because `telemetry-calibrate`, `hook-cost` and `weekly-report` read only the live path, so an undeclared trace was swept at every init and devt's own cost analytics saw one workflow at a time. | ✓ |
