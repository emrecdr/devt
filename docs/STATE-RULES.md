...
4. **Ad-hoc bucket sweep** (cleanupStateFiles) — `init.cjs` calls `cleanupStateFiles({ staleDays: 1, adHocStaleDays: 1, adHocCutoffMtime: <prior_workflow_created_at> })`. The `adHocCutoffMtime` opt (when set) takes precedence over `adHocStaleDays`; `init.cjs` reads `workflow.yaml::created_at` BEFORE the strip+restamp and passes it as the cutoff so anything ad-hoc older than the PRIOR workflow's start gets archived. Falls back to `adHocStaleDays=1` calendar-age gate when `created_at` is unavailable. Catches multi-PR-per-day residue (greenfield calibration #10: ~16 files from prior same-day sessions that escaped the calendar-age gate). Anything classified as `ad_hoc` by the audit (not in canonical + pattern_allowed + ephemeral) and older than the cutoff is archived; recent ad-hoc files (current-session work-in-progress) are preserved.
