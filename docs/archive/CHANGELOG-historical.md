# devt — Historical Changelog (v0.1.0–v0.162.0)
...
A six-lane parallel review (receipt-validated against the run's on-disk artifacts: workflow.yaml chain, MCP trace, lane files) surfaced six defects. Three shared one root cause nobody had named: **workflow_id rotates on every workflow_type transition, but consumers matched artifacts against only the current (or current+original) id** — the run's chain was six ids deep, so a live lane classified "foreign" and was dropped from consolidation, and three real `get_neighbors` MCP calls counted as zero, flagging honestly-executed drill-downs as "fabricated". One reported defect turned out to be the reporter's shell, not devt's writer — reproduced and corrected in the field before implementation.

- **Chain-aware id matching (one helper, two surfaces).** `workflowIdChainSet()` accepts the whole session chain (current + original + `workflow_id_history`), bounded by `first_created_at` — membership alone can't vouch for freshness because history survives `reset-soft`, so lane files need `mtime >= anchor` and trace records need `ts >= anchor`. Consumers: `list-lane-outputs` cid classification (a lane stamped under an intermediate rotation now stays "current"; outside-chain and pre-anchor stay "foreign"; new `cid_prefix` field makes exclusions diagnosable) and `assert-graphify-decision`'s fabricated-drill-down count. The consolidate step now names dropped lane IDs with recovery instructions instead of printing a bare count.
- **CLI fallback writes trace records.** The documented oversize fallback (`graphify neighbors --max-bytes`) produced zero trace evidence, so an all-CLI drill-down set was indistinguishable from a hand-typed one. `graphify neighbors` and `graphify blast-radius` now append records to `_mcp-trace.jsonl` (tool `cli__graphify__get_neighbors` / `cli__graphify__blast_radius` — the gate's existing matcher accepts them) with workflow context, session discriminator, result size, and a correlation_id echoed as `_meta` in the CLI output so drill-down headings can cite `[call: <id>]` exactly like MCP responses.
- **zsh-safe JSON piping, workflow-wide.** The field's "invalid escape in preflight-brief.json" was reproduced as zsh's builtin `echo` eating one backslash level from valid JSON mid-pipe (review-weight's regex-source `risk_surface` patterns trigger it; the brief and the writer were innocent — the reporter re-attributed it themselves). All 230 `echo "$VAR" | jq` sites across 16 prose surfaces are now `printf '%s\n' "$VAR" | jq`.
- **Working-tree scope union in workflow prose.** The CLI layer already collected committed range + working tree + untracked; `scope_check` and `identify_scope` still counted `base...HEAD || HEAD~1` — empty exactly when the review target is uncommitted work. Both now consume the new `state changed-files [--base=]` CLI (defaults to `git.primary_branch`, then main).
- **devt-graphify MCP parity.** `blast_radius` now applies the 60KB shape-aware default cap + `max_bytes` schema override (the cap had shipped on the devt-memory server only; field-measured 100,280-byte response landed verbatim in orchestrator context — the very payload that pushed the operator onto the CLI fallback path), and its trace records carry the session discriminator that had likewise shipped memory-side only.
- **Auto-reset on completed prior.** Back-to-back same-type reviews always prompted (auto-reset required task+type change). A COMPLETED prior (phase=complete, active off, not paused) older than 1h now auto-resets with no task-change requirement — task equality is fuzzy string comparison that false-negatives on rephrasings, and a finished workflow's counters deserve rotation either way. Interrupted and paused workflows still prompt; `status: DONE` alone is a per-phase marker and never treated as completion.
- Gates **K260–K266** (chain-aware cid behavioral, chain-aware gate behavioral, zsh-piping drift, changed-files union behavioral + prose wiring, MCP parity, completed-prior auto-reset behavioral incl. paused protection, CLI trace behavioral) + **K136 recalibrated** (behavioral assertion now runs the union CLI against its 3-commit fixture, adds an untracked file to the required capture). Drift-guard stack 166 → 173 deep (K94–K266).
