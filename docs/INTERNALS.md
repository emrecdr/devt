...
**Layer-1 — inline mechanical claim-check.** `state assert-artifact-present <agent>` reads agent's `outputs.primary` from `agents/io-contracts.yaml`, asserts `[ -s file ]` exists. Every Layer-1 call wraps result through `persistClaimCheckResult` which appends a record to `.devt/state/claim-checks.jsonl` (`{ts, source:"claim_check", agent, verdict:"success"|"failure", reason, expected_path, workflow_id}`). Workflow .md bash blocks call Layer-1 immediately after each output-writing dispatch — prints `[BLOCKED]` warning but does NOT halt (advisory inline).

**Layer-2 — post-hoc finalize gate.** `state assert-claim-checks-resolved` reads `claim-checks.jsonl`, groups records by agent, computes per-agent latest verdict in workflow window (anchored at `created_at`). Failure with no subsequent success blocks. **Resolution semantic**: a successful re-run after a failed dispatch OVERWRITES the failure record (orchestrator re-dispatched successfully → no longer unresolved). Honors `claim_check_mode` config knob (block default, mirrors `dispatch_hygiene_mode` pattern). Wired in all 4 workflow finalize sites adjacent to `assert-no-raw-dispatches-this-session`.
...
**Delivery mode.** BOTH delivery pipelines resolve `dispatch.rules_mode` / `dispatch.rubric_mode` (default by-reference): `dispatch render-filled` swaps every `governing_rules.content` body (and the inline rubric) for a read-from-disk stub (per-call flags win; `--inline-rules` restores full inlining for worktree-isolated dispatches), and the init compound payloads deliver the same stubs in `$CTX.init.governing_rules.content` (`delivery_mode` surfaced, `stubbed_bytes_saved` counted) so the canonical LLM-fill workflow dispatches are by-reference too. The stub text is single-sourced (`init.cjs::RULES_BY_REFERENCE_STUB`). By-reference delivery bypasses the 96 KB inline byte-cap (`loadGoverningRules` `inlineByteCap` option, threaded from both pipelines) — the cap protects dispatch size, so it applies only when bodies actually inline; in by-ref mode every rules file gets a stub regardless of size. The `<context_loaded_contract>` is template-static with structurally-conditional wording — stubs mean read-and-record in `## Context Loaded`, inline content means neither — riding in every governing-rules-carrying envelope in both modes; no render-time injection. Missing rules files resolve to the `(no <path> available — file not present in this project)` fallback (replacer-level for renders, fill-prose grammar for LLM-fill) instead of leaking literal placeholders. Agents recognize the `(by-reference: …)` stub as an instruction to Read from disk, not as content (stub-awareness clause in each consumer agent's context_loading).
...
**Delivery mode.** `dispatch.guardrails_mode` gates delivery, mirroring `rules_mode`: default `by-reference` swaps each body for a single-sourced `GUARDRAILS_BY_REFERENCE_STUB` read-from-disk directive (which the templates' Context-Loaded contract treats as read-and-record); `inline` keeps full bodies. `--inline-rules` (and `--guardrails-by-reference` for the inverse) override per dispatch; `--inline-rules` forces inline for worktree-isolated dispatches. Default is `by-reference` — field-measured ~6.1K tok saved per programmer dispatch (~3.9K reviewer, ~3.3K tester), on the same proven stub mechanism `rules_mode` uses.
...
| `replay-orchestration.cjs` | Orchestration replay harness — records `.devt/state/` fixtures and drives the real producer/gate CLI sequences the workflow prose invokes, asserting block/advance decisions (turns `field:` scar-tissue notes into regression tests; pure CLI, no LLM). Distinct from `test-gates.cjs` (single-gate unit tests) — this replays producer→gate seams. Called by `smoke-test.sh` (K337). | **CI** (via smoke-test) |
