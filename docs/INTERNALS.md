...
**Unified gate-trace.jsonl observability.** `bin/modules/state.cjs::traceGate(name, fn)` wraps every `assert-*` CLI subcommand in the `run()` switch. Records appended: `{ts, source:"gate_trace", gate, verdict:"ok"|"warn"|"fail", reason, workflow_id, phase}`. Cal #18 #4: gives unified observability across the entire gate surface in one file instead of stitching together `dispatch-warnings.jsonl` + `claim-check-failures.jsonl` + `preflight-denies.jsonl`. Query patterns: `jq -s 'group_by(.gate) | map({gate: .[0].gate, fires: length, blocks: map(select(.verdict=="fail")) | length})'`.
