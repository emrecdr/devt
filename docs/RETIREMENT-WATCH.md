...
| `scripts/check-docs.sh` | none — doc-completeness checker for a user project's `.devt/rules/documentation.md`, invoked by no workflow, agent, skill, or CI surface | **RETIRED** — script deleted; revivable receipt-gated if a docs workflow wants a deterministic completeness leg |
...
- **In-process hook fast path** (run-hook.js requires the guard module directly instead of spawning bash + a second node): measured chain is runner-node 26ms + bash ~8ms + CLI-node 39ms = 73ms warm per bash-guard fire — in-process saves ~45ms/call, ~1–2s/day at observed frequency, at the cost of a second execution model inside the runner. TRIGGER: a field receipt showing sustained warm p50 > 250ms on a PreToolUse guard, or guard frequency an order of magnitude above today's.
- **PreCompact thread-snapshot hook** (auto-fire the `/devt:thread` distillation before the harness compacts a long session): the platform event exists and devt has no hook on it — but devt's file-based state is DESIGNED to survive compaction (`workflow.yaml` + `.devt/state/` artifacts are what `/devt:next` resumes from), so the hook only adds value if compaction demonstrably loses routable context that the state files don't carry. TRIGGER: a field receipt showing a mid-workflow compaction after which `/devt:next` mis-routed or the operator had to reconstruct context by hand (RECEIPT-21 asks for exactly this).
