...
node bin/devt-tools.cjs memory candidates-footer         # one-shot finalize-footer: status + threshold + cooldown + emit + touch in one call. Used by code-review.md, code-review-parallel.md, quick-implement.md::finalize, dev-workflow.md::finalize (replaces a 7-line inline bash block previously duplicated across all four — round 5 cut). workflows/next.md keeps the lower-level candidates-status primitive because its variant uses ready_to_surface as a shell variable to gate a downstream AskUserQuestion. The Stop hook consumes the same logic in-process (`state.cjs::stopHook` → `candidatesFooterStatus`) — the session-end curation surface for sessions that never hit a finalize step (→ docs/HOOKS.md, Session-End Curation Surface)
...
All candidates flow into `.devt/memory/_suggestions.md` (NEVER auto-promoted), with a **recurrence ledger** (`_suggestions-ledger.json`) counting how many harvests have staged each candidate — paraphrases fold into one entry via the same 0.6 token-overlap similarity the dedup uses. The curator asks ONLY about candidates seen `memory.promote_recurrence_threshold`+ times (default 3); below the bar they build evidence silently and appear as one summary line. The ask itself is **plain language** — "You've preferred this approach N× across sessions: {one plain sentence}. Make it a standing project rule?" with three options (make it a rule | don't suggest again → REJ tombstone | not yet — keep watching). The curator decides ALL mechanics silently (doc type, active-vs-candidate status via the tooling-evolving classifier, affects_paths, keywords) and reports them after the choice; the original reasoning rides verbatim below the options as secondary context. An explicit `/devt:memory promote <id>` bypasses the recurrence gate — the user already chose that candidate. Only on user approval does the markdown file get written.
...
| `memory.shared_roots_coerce` | `false` | Trust tier: shared (non-local) roots always govern and advise, but their docs trigger block-mode pre-flight **denial** only when `true`. Edits governed solely by shared-root docs log a `shared-advisory` scratchpad line and proceed. |
...
**Multi-root shared roots are the exception, so trust them accordingly.** Curator writes always go local; shared roots are read-only from devt and edited directly by their maintainers, so **shared-root docs never pass the curator gate** — a change in a shared root re-governs every consuming project on the next `memory index` / `memory-auto-index` with no per-project review, though the change is now *surfaced* (not blocked) by the shared-root delta in the index result, `health`, and the auto-index hook line. A shared-root ADR governs and advises with the same authority as a locally-curated one, but under block-mode pre-flight it coerces only when `memory.shared_roots_coerce: true` (default `false` — an edit governed solely by shared-root docs logs a `shared-advisory` scratchpad line and proceeds; any local governing doc keeps the full deny path). A shared-root REJ tombstone still suppresses proposals across all consumers, attributed to its root in the Brief (`_(shared:<label>)_`). Add a shared root only when you trust its contents as much as your own curated memory; enable coercion only when you'd grant it commit-blocking authority. Normal use (an org ADR repo behind PR review) is low-risk; the caution is for roots outside your review control.

**Provenance is legible at the governance surface.** Shared-root docs are marked in the Brief's governing lines (`_(active·verified·shared:<label>, lane B)_` — the label is the root's basename, parent-qualified only on basename collision) and in the sidecar's `governing[]` (`shared_root: "<label>" | null`). Local docs render unchanged, so single-root projects see zero provenance noise. **The trust tier completes the model:** block-mode denial tiers on provenance (`memory.shared_roots_coerce`, default `false`) — shared roots advise without coercing unless explicitly granted — and REJ suppression is root-attributed in the Brief and recommendations. Together with the provenance markers and the shared-root index delta, the full mitigation sequence for the multi-root bypass is in place; what remains inherent is that shared-root content itself never passes the local curator gate, which is the documented trust decision above.
