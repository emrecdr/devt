...
node bin/devt-tools.cjs memory candidates-footer         # one-shot finalize-footer: status + threshold + cooldown + emit + touch in one call. Used by code-review.md, code-review-parallel.md, quick-implement.md::finalize, dev-workflow.md::finalize (replaces a 7-line inline bash block previously duplicated across all four — round 5 cut). workflows/next.md keeps the lower-level candidates-status primitive because its variant uses ready_to_surface as a shell variable to gate a downstream AskUserQuestion. --hint-only (Stop-hook mode, hooks/stop.sh): emits ONLY the 💭 hint when ready, silent otherwise — the session-end curation surface for sessions that never hit a finalize step (→ docs/HOOKS.md, Session-End Curation Surface)
