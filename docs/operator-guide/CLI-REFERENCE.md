...
node bin/devt-tools.cjs state register-lane --id=L1 --scope=<community> --files=a.py,b.py [--repo-root=<dir>] [--base=<ref>] [--overwrite]
...
# Layer-1 mechanical claim-check. Reads agent's outputs.primary from agents/io-contracts.yaml, asserts the file exists + is non-empty. Returns {ok, agent, expected_path, exists, size_bytes, reason}. Every call persists result to .devt/state/claim-checks.jsonl for Layer-2 consumption. Workflow runners call after each output-writing dispatch to verify "agent claims it wrote X" against ground truth. Polymorphic form: `assert-artifact-present <agent>:lane-<id>` resolves expected_path from `workflow.yaml::lanes[].review_file` instead of io-contracts (used by code-review-parallel for per-lane Layer-1 records — each lane persists a distinct stream within the workflow window)

node bin/devt-tools.cjs state assert-claim-checks-resolved
# Layer-2 post-hoc finalize gate. Reads claim-checks.jsonl, computes per-agent latest verdict in workflow window; failures with no subsequent success block. Resolution semantic: successful re-runs overwrite prior failures. Honors claim_check_mode={block|warn|off} (default block, mirrors dispatch_hygiene_mode). Wired into all 4 workflow finalize sites adjacent to assert-no-raw-dispatches-this-session
...
```bash
node bin/devt-tools.cjs dispatch render-filled <agent>:<workflow_id|auto> [--rules-exclude=heading,list] [--notes-file=<path>] [--out[=<path>]] [--inline-rules | --rules-by-reference --rubric-by-reference]
# Delivery mode: governing_rules bodies + the inline rubric ride BY-REFERENCE by default (config `dispatch.rules_mode`/`dispatch.rubric_mode`; explicit flags win over config) — each sub-tag carries a read-from-disk stub, the Context-Loaded contract is auto-injected (agents record what they actually Read; the verifier checks reads cover cited rules), rules_hash keeps drift detection, and CLAUDE.md is dropped outright because the harness auto-injects it into every subagent. --inline-rules restores full inlining of both — required for worktree-isolated dispatches whose disk view may not match the orchestrator's.
# Render an envelope with state-driven placeholder substitution. Defaults from active workflow.yaml when :auto. Opt-in --rules-exclude strips matching `## Heading` sections from inlined governing_rules.content before substitution — exact title match, predictable. Auto-wires from project config: `.devt/config.json::rules.exclude_sections: []` is merged with the CLI flag list (deduped) so the section strip accrues per-project without per-call plumbing. `render-lanes` threads the merged list through to every lane envelope. Envelope carries a trailing `<!-- rules-excluded: N sections (X.X KB saved) -->` marker for audit. Typical savings run 15-35% per dispatch depending on which sections are excluded. Consolidator support: rendering code-reviewer:code_review_parallel pre-fills {lane_files_newline_separated} from the lane registry (terminal lanes only, foreign cids excluded — same filter the consolidate step uses), and --notes-file injects its content as an <orchestrator_notes> context block (cross-lane reconciliation directives, validation evidence) so run-specific judgment never requires hand-rolling the envelope. Unreadable --notes-file is a loud exit-2 error, never a silent omission. Every rendered envelope carries a minted <correlation_id>cid_<wfprefix>_<agent> tag (render-lanes stamps per-lane cids; render-filled previously minted none — the gap that made a compliant consolidator dispatch read as raw). --out writes the envelope to disk (bare --out → canonical .devt/state/dispatch/<target>.txt) and prints a ~200-byte pointer STUB instead of the body: cid tag + envelope path + read-and-execute contract. Pointer dispatch is first-class — field-measured ~50K output tokens saved on a six-lane run with envelopes never entering orchestrator context; the guard passes on the cid content, not a spoofable path pattern. render-lanes --out mode emits the same stub per lane in its JSON summary

node bin/devt-tools.cjs dispatch warnings resolve <warning_id> --reason="<text>" [--evidence="<text>"]
# Scoped per-record remediation for a warning that flagged a substantively compliant dispatch (e.g. a pointer dispatch that consumed a rendered envelope). Appends a source='resolution' ANNOTATION to dispatch-warnings.jsonl — the original record persists (auditable, like update-lane override_reason); never deletes, never clears by type (a legitimate warning landing between list and a blanket clear would be silently absolved). --reason mandatory; unknown ids and double-resolution rejected. Consumed by `state assert-no-raw-dispatches-this-session`: resolved-with-reason records stop counting, so one false positive never again forces re-dispatch, config-wide warn-mode, or --skip-gates (which bypasses EVERY gate on the transition). The hygiene hook stamps warning_id per raw_dispatch record at write time; the gate's unresolved[] output carries the ids to target. Axis-H sections render resolutions as 'resolved: <reason>'

node bin/devt-tools.cjs dispatch render-lanes [target] [--target=<agent>:<workflow>] [--out=<dir>] [--only=<L2,L4>] [--lane-<id>-focus=<text>]
# Emit per-lane envelopes for every entry in workflow.yaml::lanes[]. Default target is code-reviewer:code_review (the canonical per-file review template carrying the self-grade directive in its task body — hand-rolled raw-dispatch task text consistently omits the self-grade directive, so emitting envelopes from the canonical template by default makes the bypass structurally impossible). Each lane gets the base envelope + injected <lane_id>, <lane_community>, <lane_files>, <memory_affects>, and — when the lane has a diff_artifact — <lane_diff> + <lane_method> (diff-first review: "the diff IS the change under review, full files only for context"; chunked/split lanes additionally get the hunk-enumeration read strategy) before </context>; canonical "Write review to .devt/state/review.md" trailer is overridden per-lane to lane.review_file so concurrent lanes don't clobber one path. Stdout mode: concatenated with `<!-- LANE: <id> -->` separators. --out=dir mode: writes one file per lane + returns JSON summary with byte counts. --only=<ids> renders just the named lanes against the FULL registry (neighbor ownership intact; unknown id is a loud exit-2 error, never a silently-empty render) — the redispatch_lanes retry path, composed with --lane-<id>-focus for the narrowed ask, so retries carry the same machine-built context blocks as round 1 (a hand-assembled field retry dropped scope_trust/scope_hint/memory_signal). Filtered results carry only[] + registered_count. Empty-state path: clear stderr message + usage hint before exit 2
