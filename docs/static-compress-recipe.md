...
2. **`headroom` probe** — if `headroom --version` succeeds on `PATH`, the file content is piped through `headroom compress -` (neural extractive, ~40% reduction at 7.9/10 Claude-judged fidelity per the chopratejas/kompress-base model card).
3. **Regex fallback** — `prose-shrink.cjs` (caveman-shrink port) runs with sentinel-protected segments. ~25-35% reduction, fully deterministic, zero dependencies.
