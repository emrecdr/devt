...
function run(subcommand, args) {
  switch (subcommand) {
    ...
    case "snapshot-lanes":
      return snapshotLanes();
    case "assert-lanes-unchanged":
      return traceGate("assert-lanes-unchanged", () => assertLanesUnchanged());
    case "council-trace":
    ...
    case "partition-lanes":
      return partitionLanes({
        targetLanes: parseInt(_getFlag(args, "--target-lanes") || "5", 10),
        base: _getFlag(args, "--base"),
        range: _getFlag(args, "--range"),
      });
    case "lane-sample":
      return laneSampleForVerification();
