 ...
 *
 * Effort is NOT routed here: agent frontmatter `effort:` is the only effort
 * lever the harness honors — the Agent tool exposes a per-call model
 * parameter but no per-call effort, so profile-differentiated effort
 * routing structurally cannot reach a dispatch.
 */
