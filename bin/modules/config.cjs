...
function findProjectRoot() {
  ...
  while (dir !== path.dirname(dir)) {
    if (
      fs.existsSync(path.join(dir, ".devt.json")) ||
  ...
  process.stderr.write(
    JSON.stringify({ warning: "No .devt.json or .git found; using cwd as project root: " + process.cwd() }) + "\n"
  );
  return process.cwd();
}
...
module.exports = { run, getMergedConfig, findProjectRoot, deepMerge };
