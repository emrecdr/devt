...
name: scratchpad
description: >-
  Cross-agent within-workflow notes — one agent records a decision, observation, error, or blocker for a
  later agent in the same workflow. Use when the user says "note this for later", "record decision", "pass
  context to next agent", or "log this for the next step". Persists in .devt/state/scratchpad.md
