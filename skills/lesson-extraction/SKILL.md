...
description: Capture lessons from completed work — what went wrong, what worked, non-obvious discoveries, patterns worth remembering. Produces structured drafts in `.devt/state/lessons.yaml` with categorical confidence (verified/explicit/inferred/observed/speculative); curator gate-promotes drafts into `.devt/memory/lessons/LES-NNNN.md`. Trigger on 'what did we learn', 'capture this lesson', 'remember this for next time', 'key takeaway', 'extract lessons from this session', incident reflections like 'the real fix was X not Y' or 'this is a landmine worth recording', and end-of-session retrospectives. Produces drafts only — does NOT query existing lessons (use `memory query`), does NOT write permanent storage (curator does via AskUserQuestion), and does NOT improve the plugin system (use autoskill for rule/skill/agent changes).
