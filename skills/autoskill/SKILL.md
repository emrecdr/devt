...
If the same advice would apply to any project, it does not belong in a skill or `.dev-rules/`.
...
**Update project rules** (`.dev-rules/` or `CLAUDE.md`) when:
...
- "We use UUIDv7 for all entity IDs" → `.dev-rules/coding-standards.md` (project convention)
- "Auth logic lives in middleware, not handlers" → `.dev-rules/architecture.md` (architecture decision)
...
target: skills/codebase-scan/SKILL.md  # or .dev-rules/coding-standards.md, agents/programmer.md
...
- The change is not already covered by an existing skill or `.dev-rules/` file (search first)
...
Present `.dev-rules/` changes before skill changes (project context first). Wait for explicit approval before editing any file. Never implement proposals without approval.
...
- **Feeds into**: Skill files (agent behavior), `.dev-rules/` (project conventions), agent files, workflow files
