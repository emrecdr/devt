---
name: autoskill
description: Detect recurring correction patterns and propose concrete improvements to the devt plugin system — updating skills, agents, `.devt/rules/`, or workflows. Use when the user has corrected the same behavior multiple times and wants to codify it as a permanent rule or skill update. Trigger on 'improve the plugin', 'add a rule for this', 'we keep making this mistake', 'this should be a skill', 'codify this pattern', 'the agent keeps forgetting to X', 'analyze this session for corrections', or when repeated course corrections reveal a skill gap. Improves the plugin SYSTEM (rules, skills, agents, workflows) — NOT about recording lessons (use lesson-extraction) and NOT about archiving stale lessons (curator handles via `status: superseded`).
