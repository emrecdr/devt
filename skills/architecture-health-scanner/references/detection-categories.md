...
**Investigation**: Check `.dev-rules/coding-standards.md` for the project's error hierarchy. A generic `catch Exception` above domain-specific errors silently converts business errors into 500s. Every custom exception should be traceable to the project's error base class.
...
**What**: Code that uses outdated patterns, deprecated APIs, or legacy conventions when the project has established newer standards. Detected by comparing code against `.dev-rules/coding-standards.md` and `.dev-rules/patterns/common-smells.md`.

**Typical severity**: Minor to Important

**Investigation**: Read the project's coding standards and common smells documentation. Scan for patterns they explicitly list as forbidden or deprecated. This is project-specific — the detection commands are defined in `.dev-rules/`, not here. Common examples: deprecated framework APIs, legacy library patterns, outdated DI styles.
