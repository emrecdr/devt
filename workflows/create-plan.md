...
<prerequisites>
- `.dev-rules/` directory exists with coding-standards.md, architecture.md, quality-gates.md
...
Before dispatching agents, check `.devt.json` for `agent_skills.<agent_type>`. If not configured, consult `${CLAUDE_PLUGIN_ROOT}/skill-index.yaml` for defaults:
