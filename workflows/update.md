...
Manual update: claude plugin add --from github emrecdr/devt
```
STOP here.
...
**Type: `plugin`** (installed via `claude plugin add`)
...
Could not determine install type. Update manually:
  Option 1: claude plugin add --from github emrecdr/devt
