...
> **If you delegated to `code-review-parallel.md` at `scope_check`, STOP HERE.**
> Everything below this line — including the three `SHARED-STEP` pointers, which
> carry `MODE=single` — is the single-dispatch tail and does not apply. The
> parallel workflow owns the rest of the run and re-enters the same shared steps
> with `MODE=parallel`. Both files are loaded at once and their pointers name the
