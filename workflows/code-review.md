...
**Empty drill-down handling**: when `get_neighbors` returns `results: []` for a top-3 dependent (e.g., a module-level container where callers are dynamically dispatched), FIRST check the response's `dropped_by_file` aggregation — when filtering leaves nothing, the drops ARE the caller set (DI wiring and router consumers classified as noise live there, aggregated per file); write the drill-down from it instead of recording an empty section. Only when `dropped_by_file` is also absent/empty, record `## Drill-down: <SYM> (empty — dynamic dispatch suspected) [call: <correlation_id>]` and substitute the next-ranked dependent in the cap-3 slot. Bounded: try up to 5 ranked dependents before giving up on completing the top-3.
