...
Task(subagent_type="devt:architect", model="{models.architect}", prompt="
  ...
  <context>
    <files_to_read>
      .dev-rules/architecture.md — layer rules, module boundaries, dependency direction
      .dev-rules/coding-standards.md — coding conventions, forbidden patterns, entity standards
      .dev-rules/testing-patterns.md — test structure, coverage rules, soft-delete testing requirements
      .dev-rules/golden-rules.md (if exists) — non-negotiable project rules
      .dev-rules/patterns/common-smells.md (if exists) — project-specific anti-patterns with detection commands
      CLAUDE.md (if exists)
    </files_to_read>
...
```bash
REPORT_DIR=$(node -e "const c=JSON.parse(require('fs').readFileSync('.devt.json','utf8'));process.stdout.write((c.arch_scanner&&c.arch_scanner.report_dir)||'docs/reports')" 2>/dev/null || echo "docs/reports")
mkdir -p "$REPORT_DIR"
