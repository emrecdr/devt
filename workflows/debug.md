...
<prerequisites>
- `.dev-rules/coding-standards.md` exists (for code context)
- `.dev-rules/quality-gates.md` exists (for verification after fix)
...
Before dispatching the debugger agent, check `.devt.json` for `agent_skills.debugger`. If not configured, consult `${CLAUDE_PLUGIN_ROOT}/skill-index.yaml` for defaults (codebase-scan).
</agent_skill_injection>
