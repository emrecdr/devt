...
```bash
CTX=$(node "${CLAUDE_PLUGIN_ROOT}/bin/devt-tools.cjs" state workflow-context-init --workflow-type=quick_implement --scope="${TASK_DESCRIPTION}" ${PRIMARY_BRANCH:+--primary-branch=$PRIMARY_BRANCH})
...
```bash
# KEEP IN SYNC (shared contract gated by smoke gate K320): mirrored in dev-workflow.md test step — the load-bearing mechanics (post-dispatch-check, read-sidecar, phase update) must match; prose + mode-specific blocks may differ freely
