...
<prerequisites>
- `.dev-rules/quality-gates.md` exists with gate definitions
...
Read `.dev-rules/quality-gates.md` and extract all defined quality gates.
...
If `.dev-rules/quality-gates.md` does not exist:
- Check if `CLAUDE.md` defines quality commands (e.g., `make quality`, `uv run ruff check`)
...
4. **STOP: architecture** — If no quality gates are defined anywhere (no `.dev-rules/quality-gates.md`, no `CLAUDE.md` gates), STOP with NEEDS_CONTEXT.
</deviation_rules>
