...
The third call auto-fires the **Topic Pre-Flight Brief** — Lanes A-H + blast radius for the feature topic. Surfaces governing ADRs/Concepts/Flows so the PRD aligns with permanent architectural truth, AND surfaces REJ tombstones so the spec doesn't propose pre-rejected approaches (e.g. spec asking for Redis sessions when REJ-001 tombstoned that). The Brief lives at `.devt/state/preflight-brief.md`. Skip silently on failure.

Load project context:
- Read `${CLAUDE_PLUGIN_ROOT}/references/questioning-guide.md` — how to interview effectively. Pay particular attention to "Before You Ask" (codebase-first), "Walk the Decision Tree" (dependency ordering), and "One at a Time" (AskUserQuestion sequencing) —+
...
- **Blind-spot round (last, before Step 4)**: after the category walk, ask yourself — given the user's stated expertise level and what the codebase analysis surfaced, what unknown-unknowns has this interview not touched? Surface the top 1-3 as ONE final AskUserQuestion (each option names the blind spot + why it could bite), or record "no blind spots surfaced" in the PRD. Unknown-reduction up front is cheaper than enforcement mid-flight.
...
**Read `${CLAUDE_PLUGIN_ROOT}/references/prd-template.md` now** for the PRD template, and fill ALL sections with real interview answers — no placeholder brackets, every section populated from the interview.
</step>
...
Best practices, anti-patterns, and a worked example session for `/devt:specify` live in `${CLAUDE_PLUGIN_ROOT}/references/specify-guide.md` — Read it for interview technique + a PRD example when useful (not required to execute the steps above).
