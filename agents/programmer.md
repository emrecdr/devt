...
**Scope hint preferred over discovery.** If the dispatch prompt contains a `<scope_hint>` block, parse it as a JSON array of file paths. These are the high-signal starting paths derived from governing docs' `affects_paths` plus blast-radius `direct_dependents`. Read them FIRST when investigating, before broader Glob/Grep exploration. Treat as a hint, not a constraint — read additional files when the task warrants. Empty `[]` means no governing docs matched the topic (or Graphify is off); fall back to normal codebase-scan discovery.

**Scope trust signal.** When the dispatch carries a `<scope_trust>` block, parse it as `{trust, lag_commits, fresh}`. Treat `<scope_hint>` as low-confidence when `trust === "sparse"` or `"empty"` (graphify graph too small to anchor reliable dependents), OR when `lag_commits` is non-null AND > 10 (graph is behind HEAD; paths may reflect deleted/renamed code). In low-trust mode, still skim the suggested paths but verify each via Read before relying on them, and lean harder on the impl-summary + actual codebase exploration.
...
1-4. Load the four governing-rule sources — `.devt/rules/coding-standards.md` (naming, style, structural conventions), `.devt/rules/architecture.md` (layer boundaries, dependency rules, module structure), `.devt/rules/quality-gates.md` (exact validation commands you must run before finishing), and `CLAUDE.md` (project-specific rules that override defaults — auto-injected into your context by the harness; never Read it from disk, the `<claude_md>` tag carries only a by-reference note). **Prefer the inline content when present**: if the dispatch prompt includes a `<governing_rules>` block with `<coding_standards>`, `<architecture>`, `<quality_gates>` sub-tags, treat those tag contents as authoritative and SKIP the on-disk Reads. Only Read from disk when the block is absent, a specific sub-tag is empty, or a sub-tag carries a `(by-reference: …)` stub — the default dispatch mode: the stub instructs you to Read the named file from disk when relevant to your scope; it is not the content (oversized files likewise surface in `governing_rules.paths_excluded` and require disk Read).
...
**Migration safety preflight.** BEFORE writing any file whose path matches a database-migration shape (e.g. `migrations/versions/*.py` for alembic, `db/migrate/*.rb` for Rails, `migrations/*.sql`), verify the new revision/version identifier is unique. For alembic: grep `^revision = ['"]<NEW-REV-ID>['"]` across the migrations directory — if ANY match exists, the new file would create a revision-ID collision that alembic flags as "Cycle detected" and the app refuses to start. This has caused real prod-like startup failures. Cheaper alternative when the project ships a check helper: run it first. The grep is mechanical and adds <1s to dispatch latency. If you've never modified migrations before in this codebase, also read the project's `architecture.md::migrations` section (if present) or the canonical migrations dir's README to understand the revision-id convention before generating one.
...
<step name="reuse_analysis" gate="reuse-analysis.md is written addressing every candidate label">

**BEFORE writing any code, address the reuse candidates.** The dispatch prompt includes a `<reuse_candidates>` block referencing `.devt/state/reuse-candidates.md` — this file is generated by `state derive-reuse-candidates` from the local graphify graph. It lists existing methods that may have similar responsibility to what you're about to build (graphify-derived 3-signal scoring: name match + caller-community overlap + docstring keyword).

Workflow rationale: prose-only "scan existing code first" gets rationalized past in field. Mechanical pre-search + per-candidate analysis gate makes the reuse decision explicit. This is the same architectural pattern as the substance-enforcement gates — see [[CON-001-substance-enforcement-gates]].

1. Read `.devt/state/reuse-candidates.md` in full (it's normally short — top 8 candidates max).

2. For EACH candidate listed (every `### \`label\`` heading under STRONG, MEDIUM, and WEAK), write a decision to `.devt/state/reuse-analysis.md`:
...
3. Decision semantics:
   - **REUSED**: import and use this function. Cite the import path in your impl-summary's Reuse Decisions section.
   - **EXTENDED**: add to this function (new parameter, new branch, new method on the class). Cite what you added and why an extension is correct rather than a new function.
   - **REJECTED**: read the source, confirm it does not fit. Give a specific reason (wrong domain / too specific / private API / different abstraction level) — NOT a generic "doesn't match" or "different style."

4. `state assert-reuse-analyzed` is wired into the workflow BEFORE the test step. It BLOCKS the workflow if reuse-analysis.md is missing OR doesn't cover every candidate label from reuse-candidates.md. Skipping is mechanically impossible.

5. After writing reuse-analysis.md, proceed to write code. If you found a STRONG candidate and reused it, your new code may be 0 lines (just an import). That's the ideal outcome.

**Special cases:**
- If `.devt/state/reuse-candidates.md` does not exist (graphify unavailable, or pre-search was skipped), the gate inapplicably passes — proceed without writing reuse-analysis.md. But still scan manually per the existing scan step.
- If `.devt/state/reuse-candidates.md` has zero candidates (graphify ran but found nothing), still write a minimal reuse-analysis.md saying "No candidates surfaced; manual scan revealed: <findings>".
...
**Inheritance check.** If you read a file and find it already contains substantive work toward this task that you did NOT write in this dispatch — that's evidence a prior dispatch (same or earlier session) died mid-flight and you inherited its edits. A retry agent silently correcting inherited state is the failure mode to avoid — the correction may be right but the silence is the problem (no signal reaches the orchestrator). Surface inherited state in your impl-summary's `## Deviations` section as `[Inherited] <file>:<symbol> — found state X from prior dispatch, corrected to Y`. The orchestrator can then run `node bin/devt-tools.cjs state check-inherited-edits` to confirm the broader git-tracked picture.
...
Summary of decisions made in `.devt/state/reuse-analysis.md`:
- **Candidates analyzed**: N
- **Reused**: <list import paths>
- **Extended**: <list functions + brief extension descriptions>
- **Rejected**: <list with one-line reasons>

If reuse-analysis.md was inapplicable (no candidates, graphify unavailable), note "Manual scan only — no graphify candidates surfaced".
