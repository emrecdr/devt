...
8. Consult `${CLAUDE_PLUGIN_ROOT}/standards/development-patterns.md` when `.dev-rules/` references patterns like Repository, Service Layer, or Guard Clause
...
3. **Fresh gate run**: Run quality gates ONE MORE TIME (not relying on earlier run):
   - Copy the exact command from .dev-rules/quality-gates.md
...
- Check the cited rule — is it in .dev-rules/ or CLAUDE.md?
...
- Finding contradicts a project rule in .dev-rules/ or CLAUDE.md
