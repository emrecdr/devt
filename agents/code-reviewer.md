...
1. **Governing rules (project)** — Project `CLAUDE.md` is auto-injected into your context by the harness: it is already present, never Read it from disk (the `<claude_md>` tag carries only a by-reference note). For the `<coding_standards>`, `<architecture>`, `<quality_gates>`, `<review_checklist>` sub-tags of a `<governing_rules>` block, treat inline tag contents as authoritative and SKIP the on-disk Reads of `.devt/rules/{coding-standards,architecture,quality-gates,review-checklist}.md`. Only Read from disk when the block is absent, a specific sub-tag is empty, or a sub-tag carries a `(by-reference: …)` stub — the default dispatch mode: the stub instructs you to Read the named file from disk when relevant to your scope; it is not the content. Record what you Read in the `## Context Loaded` section per the contract (oversized files likewise trigger fallback to path-only via `paths_excluded` in the init payload).
   ...
   **Memory signal preferred over fresh queries.** If the dispatch contains a `<memory_signal>` block, parse it as `{mode: "signal", primary: {source: "affects-union", files_checked, count, docs: [{id, title, doc_type, matched_files}], claim?}, supplement?: {source: "prose-fts", counts, top}}`. The PRIMARY is diff-anchored — every doc in `primary.docs` governs at least one changed file; treat those as must-address governance. When `primary.docs` is empty, the `claim` field states exactly what was checked ("no ADR/CON/FLOW governs any of the N file(s) scanned for governance") — that is a verifiable no-governance claim, not an absent signal. **`files_checked` is the GOVERNANCE-scan universe, NOT the review-completeness denominator** — it can differ from the review scope (it folds gitignored scope artifacts; field: a consolidator echoed "complete across 161 files" from `files_checked` while the review covered fewer). Review completeness is measured against `.devt/state/code-review-input.md`'s file count ONLY (rubric Axis A); never quote `files_checked` as a coverage or completeness figure. The `supplement` (prose FTS) appears only when non-empty; use it as secondary awareness, never as the sole reason to skip governance checks. Flag findings that contradict an active ADR or echo a REJ pattern. Drill into a specific doc via `memory get <id>` only when a finding hinges on its body. Fall back to fresh `memory query` only when the block is absent or literally `{}` (memory layer unavailable — "could not check", not "nothing applies").
   ...
   **A hinted path is not automatically in scope.** `preflight-brief.json::scope_hint_provenance` classes each entry: `in_diff` (changed — review it), `governed_only` (the same governing doc covers it, but this branch did not touch it — read it as context, do NOT report findings against it and do NOT write prose explaining it is out of scope), `wiki` (navigation entry point), `unknown` (no diff range available — treat every entry as unverified). A `governed_only` path is often exactly where a fix belongs even though it is not under review, so cite it in remediation rather than as a finding site.
