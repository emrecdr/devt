...
Comprehensive checklists for code review. Each category lists what to check, the severity level, and examples. Read `.dev-rules/coding-standards.md` and `.dev-rules/architecture.md` for project-specific rules.
...
3. If unsure whether something violates a rule, check `.dev-rules/` — if the rule exists, it is a finding
