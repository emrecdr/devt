...
Permanent ADR/Concept/Flow/REJ knowledge layer.
...
supports multi-root indexing via `memory.paths` config — index company-wide ADRs alongside project-local ones with last-wins precedence.
Use when: working with permanent architectural rules. See `docs/MEMORY.md`.

### /devt:preflight "<task>"
Generate a Topic Pre-Flight Brief listing every governing ADR/Concept/Flow + REJ tombstones + lessons + (with Graphify) blast radius.
