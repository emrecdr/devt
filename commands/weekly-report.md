...
<process>
**Mandatory first action**: read `${CLAUDE_PLUGIN_ROOT}/workflows/weekly-report.md` via the Read tool before any other action. The `@`-reference above may not be inlined by every harness; the explicit Read guarantees the workflow body is in context.
