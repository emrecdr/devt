...
if [[ "$IS_WORKFLOW_ACTIVE" == "true" && "$IS_COMPLETE" == "false" ]]; then
  # Persist stop point for resume
  node "${PLUGIN_ROOT}/bin/devt-tools.cjs" state update "stopped_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)" "stopped_phase=${PHASE}" >/dev/null 2>&1 || true
...
echo '{"hookSpecificOutput": {"hookEventName": "Stop", "additionalContext": "Workflow stopped. State preserved in .devt-state/"}}'
