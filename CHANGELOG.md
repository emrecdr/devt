...
### Cleanup pass over the two changes above — including a live mis-classification one of them introduced

**The empty-drill marker had a second reader, and widening one copy left the other wrong.** `compose-drilldowns` emits two forms of "this drill found nothing", and the substance gate was taught to accept both. Its twin — the drill-waste classifier that decides `yielded_data` — kept a private regex demanding the closing `)_`, so a DI-hinted empty drill was scored as *returned data nobody cited* (a drill-value problem) when it had in fact returned nothing (a drill-selection problem). That is exactly the conflation the classifier's own comment forbids: "they're different waste classes demanding different fixes… Collapsing them hides the lever." One module-scoped constant now serves both readers, and reverting it fails **both** gate legs together.

**`state register-lane --lens` was a silent no-op.** The flag reached `registerLane`'s signature and the batch path, but the CLI's single-lane branch never passed it — so the documented flag did nothing and the feature was reachable only through `register-lanes --from=`. A fourth instance of the half-wired class this batch exists to close, in the fix for it.

**The corpus rule moved to where every reviewer sees it.** `<search_discipline>` was injected only into lane envelopes, but all three review shapes are the same agent, and a single-dispatch reviewer deleting a symbol hits the stale-copy trap identically — `/devt:review` without fan-out is the common path, and it had no such rule. It now lives in the code-reviewer body, which also stops ~530 bytes riding every lane envelope on a fan-out (5% of a lane envelope, on the one surface where rules and rubric were deliberately moved by-reference to hold size down). The gate asserts both its presence in the agent AND its absence from the envelope, so it cannot drift back into per-lane duplication. Placing it cost the file's last budget line; an adjacent paragraph was rewrapped rather than raising the cap to fit one edit.

Also: the lens predicate is one function instead of two literal copies (a third accepted spelling added at one site would have exempted a lane from the overlap warning while leaving it without the block explaining why); the disjointness scan drops a throwaway field and a third pipeline stage; the three parallel `has*Marker` booleans become one array, so a fourth exemption is an entry rather than another `&&`; the registration row builds once and aliases visibly instead of carrying two values under two keys each; and F66's `--fresh` leg now drives its behavioral checks and its pin from one expression, closing a split where the gate validated a private copy of a regex while separately grepping for the real one.

Skipped deliberately: `listLaneOutputs`' hand-rolled block parser duplicates `parseSimpleYaml`, which the same file already calls on the same data 700 lines away and which returns every field with correct int and boolean coercion. Replacing it deletes ~25 lines and retires the "teach every new lane property a new regex" tax — but the surrounding cid-chain, id-alias and staleness logic is field-hardened, so it belongs in its own change rather than folded into a calibration batch.
