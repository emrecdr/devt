...
Tier-aware skill preloading + Agent IO Contracts registry. Two coordinated changes that shrink dispatch prefixes on lighter workflows and turn the previously-silent drift between agent frontmatter / `skill-index.yaml` / `JSON_SIDECAR_SCHEMAS` into a hard smoke-test gate.
