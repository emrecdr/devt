...
- **`pre-flight-guard.sh` deny records carry `source: "pre-flight-guard"`** (B2) — both the helper-path (`logger.cjs::appendJsonl`) and the fallback-path (direct `fs.appendFileSync` used when CLAUDE_PLUGIN_ROOT isn't set) write the discriminator. `bash-guard.cjs` already wrote the field; this aligns the two producers so `preflight-denies.jsonl` consumers can triage by source. Greenfield's 359 deny entries were all `source: MISSING` despite the v0.62.2 path-scoping fix landing cleanly.
