...
- **Commands** (19 files): thin entry points — 16 user-invocable + 3 specialized (`preflight`, `autoskill`, `council`) hidden from the `/`-menu via `user-invocable: false` but still typed-callable. Parse arguments, delegate to a workflow. Each `commands/*.md` that delegates to a workflow file loads it via an explicit mandatory `Read` in its `<process>` block — the sole, deterministic load path. The workflow body is not `@`-preloaded, so a command never drags its whole workflow family into context before routing.
...
Every dev workflow auto-fires `/devt:preflight "<task>"` at context_init. An 8-lane Topic Pre-Flight Brief surfaces governing ADRs/CONs/FLOWs/LES + REJ tombstones for the task before any code is touched. When the task references a plan file (`~/.claude/plans/*.md`), preflight auto-loads it and lifts symbols + paths from its `## Files to change` / `## Scope` / `## Symbols` sections — no need to redo the work in the task text. When graphify has discovered hyperedges (semantic multi-file groupings), preflight surfaces matches and `/devt:ship` warns if the PR touches some-but-not-all members of any hyperedge — catches the "fixed the service but forgot the readme/test/migration" failure mode automatically. A PreToolUse guard (`memory.preflight_mode = block`) blocks Edits that aren't justified by a governing ID.

**Full reference:** → [`docs/MEMORY.md`](docs/MEMORY.md) — frontmatter schema, 8 lanes, JSON sidecar, tier-aware budget, verifier memory_signal, MCP server (14 tools), multi-root config, bundle export/import, curator promotion flow, SQL views, native health checks.
...
`/devt:thread` creates session handoffs + persistent investigation contexts that survive session boundaries. `create` distills the CURRENT session (goal, context, decisions, what is left) into a uniquely slug-named thread file — referencing artifacts by path, redacting secrets, never overwriting an existing thread — and ends with a copy-paste prompt (`/devt:thread resume <slug>`) for the next session. Useful when stopping mid-task, or for multi-day debugging/research. Subcommands: `create`, `list`, `resume <N|slug>`, `update <N|slug>`.
...
**Specialized (direct-callable, hidden from autocomplete):** `/devt:council`, `/devt:autoskill`, `/devt:preflight`. The 22 advanced direct-form commands listed by `/devt:help --all` continue to work when typed (e.g., `/devt:setup --init` is the same as `/devt:setup --init`); the family-head + parameter form is the recommended entry.
...
devt/
  ...
  commands/              Slash command entry points (19 files: 16 visible + 3 specialized hidden)
...
**Honest measurement note**: in real workflows the per-dispatch wire savings are small — ~0.06–0.19% on a 37–115 KB rendered envelope in a field project with an 88.86% prompt-cache hit rate. The reason is that most of the envelope is the `governing_rules` block (the inlined `.devt/rules/*.md` corpus — project `CLAUDE.md` is now carried by-reference, since the harness auto-injects it into every subagent), and Anthropic's cache hierarchy means in-place edits to cached content invalidate downstream cache. Use the new `dispatch decompose` CLI (below) to measure your own envelopes — the empirical biggest lever per dispatch is usually selective inlining of `governing_rules`, not regex compression.
...
GitHub Actions runs `scripts/smoke-test.sh` (1000+ assertions across all CLI subcommands, including a 223-deep drift-guard stack `K94–K315` covering command stratification, parameter routing, stale-ref scans, `workflow_type` registry parity, size budgets, CLI surface contracts, telemetry-CLI input validation, push-not-pull session signal surfacing, substance-enforcement gates per `[[CON-001]]`, meta-gates that auto-validate drift count claims, CHANGELOG coverage, pipefail-trap patterns, compiled-region edit-source markers, phase-gate firing on `state update phase=X status=DONE`, intermediate-phase gate coverage, ungoverned-edit silent-skip, envelope-unavailable reason surfacing, dual-window session-signal counter, the `dispatch run` agent-launcher CLI, dispatch-helpers skill discoverability for single + fan-out cases, the `agent resume` CLI for walled-agent recovery, 3-way version coherence — VERSION ↔ plugin.json ↔ marketplace.json, dispatch usage message includes `run`, workflow-staleness warning at >24h, preflight laneG project-context enrichment, docs/GRAPHIFY.md tier-semantics positioning (symbol_anchored as canonical primary for non-GitHub), and code-review.md merge-base-aware diff resolution at identify_scope, `state reset-soft` clears per-workflow accumulators while preserving session anchors + history + memory + phase artifacts, `state staleness-check` AND-semantics (task-changed AND age>1h required to surface staleness; typo-retry and legitimate-resume both NOT stale), `dispatch render-lanes` stamps `<correlation_id>cid_*` per envelope, and dispatch-hygiene-guard matcher recognizes `<correlation_id>cid_*` as envelope-managed, model profiles carry per-agent model-tier settings (e.g. architect=opus) and opus alias resolves to claude-opus-4-8, `state assert-verifier-short-circuit` fires on substantive clean sidecar (DONE + empty self_flagged_uncertainties), self_flagged_uncertainties prompt language present in programmer.md + tester.md + code-reviewer.md, task-truncation-detector surfaces category-specific refusal hints, F5 `_isDocstringNode` threshold catches "Test X." patterns + 2-whitespace short docstrings, F4 `graphify status` surfaces lag_commits by default + node_count/edge_count under --full, F1 MCP `get_neighbors` schema declares max_bytes with server-side 60KB default, F2 `getNeighbors` filters primitives + test-path nodes with filter telemetry envelope, M4a `telemetry calibrate` CLI aggregates hook-trace and emits hook_error_pattern/hook_low_value recommendations, M3 `dispatch run-lanes` registers from `--partition=<file>` + injects per-lane `--lane-N-focus` + global `--task-suffix=<file>` + `--base=<ref>` diff base into canonical envelopes, K155 meta-gate locks `hooks/hooks.json` ↔ CLAUDE.md hook table parity, K156 meta-gate locks case-handler ⊃ default-case enumeration across every `bin/modules/*.cjs` (catches the recurring CLI-usage-drift class structurally), K157 emits preflight-brief STALE banner when artifact mtime > 4h older than workflow.yaml session anchor, K158 ensures dispatch-hygiene-guard KILL message carries per-subagent canonical CLI hint instead of a generic 3-command list, K159 verifies `getNeighbors` collapses DI-aggregation file fans (dependencies.py / wiring.py / container.py / providers.py / deps.py) to a single representative + collapsed-count marker so BFS noise from runtime-DI files no longer drowns out call-graph signal, K160 verifies `extractTopic` demotes config-enum symbols (Settings, *Config, *Backend, *Profile, LogLevel, Environment, OrderBy, ErrorCode) to the end of topic.symbols so changed-config-file diffs no longer flood scope_hint with infrastructure names, K161 verifies `symbolsInFiles` falls back to regex-extracted symbols from newly-added (un-indexed) files via the diff-hunk fallback path so symbol_anchored tier still produces anchors on additions the graph has never seen, K162 verifies `preflight laneH` surfaces auto-memory (`~/.claude/projects/<projHash>/memory/*.md`) + `.devt/state/claude-mem-harvest.md` matches at read time with correct `source` attribution so curated decisions reach the brief without bypassing the curator-write path, K163 verifies `state auto-reset-if-stale` fires only when task+workflow_type+age>24h all signal an unambiguous new working session (skips reset when workflow_type unchanged to avoid eating legitimate same-mode resumes), K164 verifies `init review --bundle` opt-in flag attaches `bundle.{preflight_generated, memory_signal_count, graphify_enabled, graphify_impact_plan, errors[]}` while leaving default `init review` behavior unchanged so existing callers don't regress, K165 verifies `graphify compose-drilldowns` emits markdown-ready drill-down sections (`## Drill-down: <symbol>` header + per-neighbor bullets + filter telemetry) for workflow concatenation into graph-impact.md so the orchestrator can't forget the drill-down step, K166 verifies `reset-soft` evicts `review*.{md,json}` + `review-lane-*.{md,json}` (review-instance artifacts that would collide with fresh-run outputs) while preserving impl-summary/test-summary/graph-impact (workflow-spanning artifacts that legitimately survive across review re-runs), K167 verifies `state list-lane-outputs` emits per-lane `cid_match` field (`current`/`foreign`/`absent`) by extracting the correlation_id from each lane file body so the consolidator can `select(.cid_match != "foreign")` against stale-cid artifacts that escaped reset-soft eviction, K168 verifies dispatch envelope template propagates `auto_memory_json` from `preflight-brief.json` so G2/laneH's auto-memory + claude-mem-harvest matches reach reviewers structurally instead of redundantly via the claude-mem harvest path, K169 verifies `assert-graphify-decision` substance gate honors the canonical empty marker `_(no neighbors found in direction=...)_` from `compose-drilldowns` output so legitimately-empty drill-down sections (DI-blind symbols, no callers) don't force operators to pad fake prose to clear the 200-byte threshold, K170 verifies `mcp-stats --strict-wid` flag scopes to current `workflow_id` only (vs default behavior that unions the full `workflow_id_history[]` chain) so workflow-scoped "Graphify activity" surfaces report THIS run's calls instead of every session back to the first reset, K171 verifies G3 strengthening excludes `config_demoted` symbols entirely from `topic.symbols` when real-symbol count ≥ 10 (FLOOR) so config-enum god-nodes don't inflate `blast_radius` effect_size on diffs with strong real signal — backfill behavior preserved when sparse < FLOOR, K172 verifies `getHyperedgesContaining` surfaces hyperedge `rationale` field at the projection (was being silently dropped despite being durable in `graph.json::hyperedges[]`) so reviewers see WHY N files belong together — the rationale layer is graphify's hyperedge feature that standard query/MCP traversal tools don't reach, so devt's direct-read bypass is the discoverable path; this gate prevents future projection-leakage of newly-added hyperedge fields, K173 verifies `state graphify-roi` computes the wasted-drill rate (drills with no review.md citation / executed drills) AND correctly excludes runs where substep 6 was skipped (no graph-impact.md → status=`no_drills_executed`, rate=null — NOT counted as 100% waste per receipt #7 explicit requirement to not punish graphify for operator skips), K174 verifies `workflows/code-review.md` impact-plan decision tree includes the `pr_scoped_diff` tier branch that activates on non-GitHub PRs (Bitbucket/GitLab/etc.) and wires `git diff <primary>...HEAD → symbols-in-files → blast_radius` so non-GitHub projects get the pr_scoped-equivalent richness they previously didn't have access to (G5 diff-hunk fallback covers new-file symbols via regex; caveat emitted for new files lacking blast_radius edge data), K175 verifies `getSymbolCollisions` filters ghost nodes (empty `source_file` AND null `source_location`) from collision results AND emits a visible `ghost_nodes_filtered` counter so the upstream AST↔semantic canonical-ID merge fix stays motivated — silent filtering would mask the root cause, K176 verifies `graphify freshness()` emits `unverifiable_freshness: true` when `graph.json` is missing the `built_at_commit` anchor so downstream staleness gates can refuse to trust an unverifiable freshness verdict (defensive surface against the receipt-#7 false-fresh leak class), K177 verifies `preflight-brief.json::suggested_reading` is a `{files, symbols}` object (cal #33.B-2 split) instead of a flat array that mixed navigable paths with bare symbol labels, K178 verifies `workflows/code-review.md::scope_check` has the operator-explicit short-circuit branch (detects parallel/single intent in `REVIEW_SCOPE` text → auto-writes scope-check-answer.txt, skips redundant AskUserQuestion when answer already in task text), K179 verifies `state mark-claude-mem-skipped` CLI writes gate-compliant content format (`reason=<enum>` line + optional `details=`) AND the resulting `claude-mem-skipped.txt` passes `assert-claude-mem-harvest`; rejects invalid reason values so the operator-declared escape valve produces gate-pass output rather than silent gate-rejection, K180 verifies the cal #34 #1 tiered staleness policy: `graphify.stale_threshold` default lowered 30→10 per receipt #8 Q4 ("a sprint's drift") + `workflows/code-review.md` carries the silent-warn band (`0<lag<threshold`) + `--no-refresh` escape hatch detection, K181 verifies `blastRadius` emits `noise_telemetry` (`filtered_test_path` + `filtered_di_aggregation` mirroring `getNeighbors`) AND `raw_direct_count` + `raw_indirect_count` so the receipt-#8-required tier-routing-shrink is auditable per [[telemetry-on-reduction]], K182 verifies `graphifyRoi` 3-state per-drill `citation` (strong/weak/none) + `yielded_data` (distinguishes drill-returned-empty from drill-returned-data-but-uncited) + dual rate (`wasted_drill_rate` strict / `wasted_drill_rate_weak` permissive) so the diagnostic delta is visible — fixed the receipt-#8-evidenced heading-parser bug that silently dropped 3 unparseable headings, K183 verifies `init.cjs::initWorkflow` rotates `dispatch-warnings.jsonl` + `claim-check-failures.jsonl` archive sidecars when closed prior workflow is detected — closes the cal #31.D auto-reset gap for same-day workflow churn that the receipt-#8-Q5 block-mode KILL-gate would otherwise false-fire on, K184 verifies `godNodes` filters `Test[A-Z]` symbol-prefix + test-path source files so pytest classes (`Test*`-prefixed) don't pollute the constitutional-abstraction list reviewers consult, K185 verifies `assert-verifier-ran` substance-checks the verification artifact body (rejects synthetic `{status:"DONE"}` skeleton; requires either verdict/findings/axes/criteria_total in the sidecar OR ≥600 substance bytes in the .md) so a well-formed empty shell can't bypass the verifier-ran safety net, K186 verifies `compose-drilldowns` has a `_findDIFactorySiteHint` defensive fallback emitting "DI factory site: <source>" when `get_neighbors` returns truly-empty results but `filtered_di_aggregation > 0` (the cal #31.B G1 collapse usually preserves a representative; the fallback covers the heterogeneous-DI-edge edge case where it doesn't), K187 verifies `cmdRenderLanes` injects a `<auto_memory>` lane-context block with a top-N name+score summary from `preflight-brief.json::auto_memory` so the G2 laneH bridge's output reaches lanes even via the hand-rolled register-lanes shortcut path, K188 verifies `laneG` accepts a `topic` parameter + call site in `generate()` passes it, expanding the FTS query token set with `topic.domains` to close the under-retrieval gap where bare `cfg.git.provider` token missed CON/REJ docs squarely relevant to the topic, K189 verifies `workflow-id-rotations.jsonl` audit log appends one line per mutation (carrying `prev_id` / `new_id` / `source` / `pid` / `argv`) for `updateState:first_activation` + `resetSoft` + `updateState:type_transition` + `initWorkflow:strip_closed_workflow` — RESET_EXEMPT so the forensic trail for "who rotated, when, via which CLI" survives the very reset events being audited, K190 verifies `state compute-impact-plan` CLI (cal #37 #3 wrapper that collapses ~115 lines of inline workflow bash into a single tier-decision call) returns `tier=skip` for not-ready graphify, `tier=pr_scoped` for GitHub PRs, `tier=symbol_anchored` for topic.symbols, AND writes `topic-symbols-dropped.json` when `topic.symbols > TOPIC_CAP=32` — preserves the verbatim tier-decision semantics so substep 5 callers don't regress, K191 verifies the `symbolsInFiles` signal-quality overhaul (cal #38.A from receipt #10): segment-boundary full-path suffix match kills cross-module basename pollution (a same-named symbol in a different module no longer leaks in), dedup collapses duplicate labels, and hunk-scoping via `git diff -U0` + defensive `source_location` parsing drops god-nodes defined OUTSIDE the changed hunk while keeping the actually-changed symbols — fixing the root cause where the impact-map was anchored on ubiquitous god-nodes instead of the changed providers), K192 verifies the god-node alarm-fatigue discrimination (cal #38.B): `blastRadius` itemizes `god_node_matches[]` with a per-symbol `ubiquitous` flag (auto-derived via degree-DOMINANCE — a hub at ≥2× the top-10 match-window floor is ubiquitous, fails safe to empty on flat distributions; overridable via `config.graphify.ubiquitous_types` with `!`-prefix force-keep) so a PR touching only ubiquitous types downgrades the ⚠️ god-node warning to an info note while a discriminating god-node still fires, plus `preflight-brief.json::god_nodes_built_at` stamps the commit the cached god-node counts were computed against so the cached-vs-live edge-count divergence (893 vs 1074) is diagnosable as staleness rather than a silent contradiction), K193 verifies the cal #38.C robustness trio: `queryGraph` falls back to token-AND-then-OR resolution (`resolution_mode` telemetry) when a multi-word query matches no single node label whole-string (was silently grep-degrading), `state disk-check` returns a warn-only `{ok, status, free_mb}` envelope (surfaces low-disk at context_init + pre-fan-out without ever blocking — user intervention is the failsafe), and `agents/code-reviewer.md` carries the incremental-write directive so a lane that runs 100+ tool calls and dies before its terminal write leaves a recoverable partial artifact instead of losing the whole analysis), K194 is the stays-general guardrail meta-gate (cal #38.D) that CI-enforces zero field-test-project coupling in the product surfaces — `bin/modules` + `workflows` + `agents` must carry neither the field project's name/path nor its distinctive symbol names, so every fix surfaced by field testing stays a general mechanism rather than hardcoding the test project (generic-term "greenfield" = a fresh codebase is deliberately not matched), K195 verifies the lane state-mutation guard (cal #37 #2): `workflow.yaml`/`workflow_id` are orchestrator-owned, so when `.devt/state/status.json` shows a fresh "running" subagent the CLI throws on `state update` (workflow_id rotation) + `state reset-soft` + `state reset` + `init workflow` strip — while `state update-lane` (the safe lane path that never touches workflow_id) stays allowed, and a stale/crashed "running" entry plus the no-status-file normal case never block — preventing the concurrent-rotation corruption where parallel lanes rotate the shared workflow_id mid-fan-out; `config.lane_state_guard` = block/warn/off), K196 verifies the shared ubiquitous-type stoplist reaches the `symbolsInFiles` G5 new-file fallback (cal #38.B item 6): an un-indexed file's regex-extracted declaration whose name collides with a ubiquitous god-node is dropped from the blast anchors (with `fallback_ubiquitous_filtered` telemetry) while a genuinely-new symbol survives — closing the gap where the stoplist was wired into god-node warnings but not the fallback path hunk-scoping can't anchor), K197 is a doc-drift meta-gate asserting `docs/STATE-RULES.md` documents every `RESET_EXEMPT` canonical filename (the set grows as forensic logs are added — `workflow-id-rotations.jsonl`, `graphify-impact-plan.json`, … — and the doc had drifted to "5 entries" at 8 actual; same structural-guard class as K117/K156/K194), K198 verifies the cal #39.A substance-check hard-fail (`checkAgentOutput` on a missing file returns `looks_like_stub:true`+`missing:true`, zero-byte returns `+empty:true` — closing the receipt-#11 race where a missing lane file silently passed the stub-grep consumers) + the `lane_failed` coverage status (distinct terminal from `deferred` so "all lanes terminal" can't hide a zero-coverage hole) + the Q5a `blast_radius` DI-opaque surface (when DI-aggregation collapse dominates the visible caller set, emit `di_opaque` + `di_collapsed_caller_files` so an empty `direct_dependents` on a Depends()-wired service reads as "DI-opaque, open these files" not "no callers"), K199 verifies the cal #39.A #4 SHA-based new-files caveat (a file whose introducing commit is an ancestor of `built_at_commit` is indexed and not counted; only files added after the build trigger the caveat — replacing the matched_files proxy that over-fired on indexed-but-symbolless files, degrading safe to "assume indexed" on unresolvable SHAs), K200 verifies the compound `state review-context-init` wrapper (cal #39.B): one call collapses the code-review context_init data-gathering (init + activate + preflight brief + memory_signal + scope-cache + freshness/eviction + impact-plan + god-node warnings) into a single bundle — removing ~15 of the orchestrator's ~19 LLM round-trips — with per-field graceful degradation (honest absence: `freshness.state` reports `disabled`/`unknown`, never a false-confident `ready`/`fresh`), a freshness-before-eviction short-circuit (a fresh re-call returns the cached bundle without re-evicting the graph-impact.md it would reuse), while the 7 gates + the staleness AskUserQuestion + the MCP impact-plan execution deliberately stay separate orchestrator stops), K201 verifies `workflows/code-review.md` context_init actually WIRES the `state review-context-init` wrapper (locking the ~19→~4 round-trip collapse so it can't silently regress to inline calls) AND the bundle carries the `init` payload (`governing_rules` + `models`) that the code-reviewer/verifier dispatch envelopes fill their `{governing_rules}`/`{models}` placeholders from — if the wrapper stopped surfacing that payload the dispatches would lose their governing rules + rubric with no other gate noticing), K202 verifies `dev-workflow.md` + `quick-implement.md` context_init wire the generalized `state workflow-context-init` wrapper (the shared `contextInitBundle` core that also powers `review-context-init`) and the workflow-mode bundle carries the `init` payload + a `memory_signal` field — locking the ~16 / ~11 inline data-gathering round-trip collapse against regression), K203 verifies the `preflight scan-prep` CLI (the `graphify_scan_prep` decision tree extracted from dev-workflow + quick-implement's 84%-identical inline bash) returns SKIP / ACTIVE / RECOVERY correctly and writes the `graphify-skip-reason.txt` decision artifact on SKIP), K204 verifies `research-task.md` context_init wires the `workflow-context-init` wrapper AND the researcher dispatch carries `<memory_signal>` (template + compiled inline) — closing the gap where the researcher investigated approaches with no REJ/ADR governance signal), K205 verifies the OTHER researcher path — `dev-workflow.md`'s COMPLEX auto-research dispatch — also carries `<memory_signal>` (generic `researcher.tmpl.md` + region-scoped compiled inline + orchestrator-prep substitution + `io-contracts.yaml` SSoT all agreeing), closing the second half of the gap K204 fixed: the `/devt:research` path was wired but the `/devt:workflow` auto-research researcher stayed blind to REJ/ADR even though the signal was already computed and cached for the programmer/code-reviewer/verifier dispatches), K206 generalizes K204/K205 into one structural class gate — `dispatch check-contracts` asserts every compiled dispatch region carries an XML block for each `context_block` its agent declares in `io-contracts.yaml`, minus that variant's documented `context_blocks_exempt` entries (e.g. `quick_implement:plan` since quick-implement skips planning; `code_review:guardrails_inline` since standalone review leans on governing_rules + the rubric), so a dispatch silently missing a declared governance block is caught for every agent+variant instead of one hand-written grep at a time — and exemptions are themselves audited (each must name a real declared block) so the suppression list can't silently neuter the gate, K207 verifies context-init freshness keys on `(scope_sig, graph_head)` jointly — a review whose changed-file signature differs from the cached bundle's recomputes instead of serving a prior review's `scope_hint`/`memory_signal`/`impact-plan` even on a fresh graph (closes the field-observed cross-PR contamination) while a same-scope re-call still short-circuits, K208 verifies `state reset-soft` evicts the scope-bound `graphify-impact-plan.json` while preserving workflow-spanning `graph-impact.md`, K209 verifies `session-start.sh` surfaces the `dispatch_hygiene_mode` safety floor when a project lowers it below the `block` default, K210 verifies the `dev-workflow.md` tier-partition is complete + disjoint (each STANDARD+ step lazy-loaded into `dev-workflow.standard.md` appears exactly once across the file set — the carve loses no step and duplicates none), K211 verifies the spine carries the `load_tier_steps` mandatory-Read directive + a `TIER-STEP` pointer for every relocated step so a STANDARD+ run can't reach an insertion point without the step body in context, K212 verifies the tier-aware `assert-verifier-ran` — dev SIMPLE/TRIVIAL opts out (no verify step) while dev STANDARD/COMPLEX requires `verification.json`, closing the pre-existing `dev.complete` hole; code_review is unaffected, K213 verifies each workflow_type's `.complete` gate set is satisfiable by that workflow — a STANDARD dev + a quick_implement each reach `complete` with only the markers their own workflow writes; regression guard for the class where a `.complete` gate required a marker the workflow never writes, which is how the code_review-only `assert-auto-curator-considered` had silently hard-blocked dev + quick_implement finalize, K214 verifies `specify.md` lazy-loads its PRD template + best-practices/anti-patterns guide from `references/` (the generate step Reads `prd-template.md`; both reference files exist with real content) — a body-weight deferral so the ~159-line scaffold isn't carried up-front on every specify run, K215 verifies `check-agent-output` stub-phrase precedence — a stub phrase is decisive only below the 300-word ceiling, so a phrase quoted inside a long substantive review no longer flags it while a short gray-zone stub is still caught (field false positive: a 2,242-word lane review flagged for one verdict sentence), K216 verifies the auto-reset double-signal condition — task-changed AND workflow_type-changed AND age>1h recommends silent reset (the prior extra >24h leg forced an interactive prompt on real session turnover), <1h typo-retry and single-signal task-only changes both stay false, K217 verifies the 7 review-pipeline phases (`scope_check` … `present_findings`) are registered in `PHASE_ORDER` — they warned "Unknown phase" on every parallel review and silently short-circuited `validateConsistency`'s phase-gated artifact checks — while a bogus phase still warns, K218 verifies `mcp-stats` never silently under-reports on id rotation: a strict `--workflow-id` query returning 0 while the `workflow_id_history[]` chain has matches emits a `hint` naming `--include-chain`, and the chain query recovers the pre-rotation records, K219 verifies `code-review.md::scope_check` derives its file count from the merge-base-aware git diff instead of the `code-review-input.md` artifact that `identify_scope` only writes later (the artifact read was always 0 on fresh runs, making the >10-file parallel trigger dead), K220 verifies `state update-lane override_reason=` appends an audit record with `prior_status` to `lane-status-overrides.jsonl` (RESET_EXEMPT) and rejects a standalone reason with no status/redispatch mutation, K221 verifies the config-drift safety banner fires once per session per project (same-session repeat suppressed via `session_id` marker, missing session_id fails loud) instead of every prompt, K222 verifies both workflows' "Graphify activity" telemetry surfaces pass `--include-chain` at all 4 `mcp-stats` call sites so context_init MCP calls stamped under the pre-rotation workflow_id stay attributed, K223 verifies `dispatch render-lanes` defaults to rules-by-reference — no rule/CLAUDE.md bodies inlined per lane (the governing-rules block was byte-identical across N lanes, 73% of a field-measured 391KB 5-lane render; the harness auto-injects CLAUDE.md into subagents anyway), `rules_hash` + read-from-disk stubs + the Context-Loaded contract present, and `--inline-rules` restores full bodies for worktree-isolated lanes, K224 verifies the Context-Loaded contract survives on the prose surfaces (lane task instruction + consolidator advisory in `code-review-parallel.md`) so by-reference selective reading stays auditable, K225 functionally validates the `evolution scan` git-history behavioral metrics on a synthetic repo — hotspot ranking (change frequency × LOC), co-change coupling detection, the changeset-size guard that keeps mass commits out of coupling, fix-density counting, and both report artifacts landing in `.devt/state/`, K226 verifies the arch-health workflow actually consumes the evolution report (evolution_scan step + `<evolution>` architect envelope block + the effort-weight instruction) so the module can't fire without value conversion, K227 verifies the `symbol_anchored` impact plan attaches a working-tree caveat when untracked or added-after-build code files are invisible to the graph (blast_radius runs against the last-committed layout — the field case ran against an untracked module's old path with no warning), K228 verifies the hunk-type census attaches `severity_calibration_note` on cosmetic-heavy diffs (comment/import/prose-only ratio ≥ `graphify.severity_note_threshold`) with the keep-the-caller-sets clause, so a popularity-driven "large" `effect_size` doesn't inflate lane severity on a behavior-neutral branch, K229 verifies `blast_radius` filters framework request/response/DI builtins (Depends/Request — framework-general set, config-tunable via `graphify.framework_builtin_noise`) from `direct_dependents`, ranks the survivors by in-degree, and emits `direct_dependents_degrees` so the drill-down top-3 anchors on structure instead of array position, K230 verifies `get_neighbors` ranks EXTRACTED edges before INFERRED and caps the INFERRED (low-confidence co-location) tail at `graphify.inferred_neighbor_cap` (default 25) with `confidence_extracted`/`_inferred_total`/`_inferred_capped` telemetry — killing the co-location noise that drowned trustworthy structural edges (field: one 247-neighbor result split 228 INFERRED / 19 EXTRACTED), K231 verifies `blast_radius` ranks drill-down targets by RELEVANCE to the diff (a dependent co-located with a changed symbol → `relevance_tier` 2, or sharing its Leiden community → 1, outranks an incidental high-fan-in god-node) and DEMOTES — never drops — god-nodes, flagging `is_god_node`/`pure_god_node` so the F16 step routes them to the `--max-bytes` fallback instead of surfacing them ahead of relevant callers, K232 verifies the evolution scan excludes test files from ALL behavioral metrics (hotspots, coupling, fix density, churn) — language-general (Python/Go/Ruby/JS/TS/Java + `tests/` dirs), config-gated via `evolution.exclude_tests` (default on, extensible via `evolution.test_path_patterns[]`) with an `--include-tests` escape hatch and a `test_files_excluded` telemetry counter, so tests that churn on every feature and co-change with the code they cover no longer inflate architectural signal), K233 verifies the skill-index buckets stay disjoint from every agent's frontmatter `skills:` — the harness preloads each agent's frontmatter skill bodies at startup, so a bucket that re-lists one makes `resolveSkills` inject a load-and-follow path for a skill already in the agent's context (a duplicate 5–14KB per-dispatch Read); generalizes the programmer-only complex-tier spot-check to all agents), K234 verifies no agent/skill body instructs the model to reproduce its own internal reasoning as response text (the `reasoning_extraction` refusal guard — newer Claude models can refuse and fall back to a heavier model when told to echo/transcribe their chain-of-thought; the pattern fires only on a reproduce-verb paired with a qualified reasoning-noun so legitimate "explain your reasoning for this finding" prose is untouched, and it self-tests against false-positive/negative so it can't rot into an always-pass), K235 verifies `blast_radius` degree coherence + filter transparency (a surviving direct dependent's `edge_count` is computed from the BFS-visited node id, not a label re-resolution that returns the first edgeless homonym — the field-observed "dependent with edge_count:0" contradiction; every filtered dependent is surfaced in a reason-coded `dropped_sample` so a wrongly-filtered real consumer is auditable instead of a black box; and when the raw dependent count is small enough to read, `raw_direct_dependents` exposes the full unfiltered set with the noise filter demoted to advisory — the gate carries a homonym so it fails pre-fix), K236 verifies `review-weight assess` is fail-safe (a small clean change is light-eligible; an auth/redaction/crypto risk-surface path, a god-node, >2 domains, or a not-small effect_size each force heavy as hard gates; a graph-blind change with no blast headline is not eligible since absence of a headline is not evidence of safety; a lockfile-heavy diff with few LOGIC files stays light — file count is the wrong trigger), K237 verifies the `memory-pre-flight` skill keeps its hot protocol inline (the `PREFLIGHT` scratchpad-line format + deny-recovery source table an agent needs mid-edit) while lazy-loading the cold detail (full Brief structure, 5-lane escalation mechanics, config, multi-root) to `references/memory-pre-flight-details.md` — the reference exists with real content and the preloaded body stays under a line ceiling so the cold detail can't creep back into all 8–9 agents' per-dispatch context), K238 verifies `graphify augment-impact-map` reproduces code-review substep-7's six deterministic sections (file+symbol god-node warnings, dropped-symbol truncation banner+section, hyperedge completeness, ambiguous bindings, and the preflight god-node fallback) byte-identically — the ~110 lines of inline `jq` that ran on every review folded into one post-MCP CLI, so the workflow body carries the call instead of the logic, and the fallback is correctly suppressed when diff-anchored god-nodes are present, K239 asserts the model surface is current and the effort lever intact — `sonnet` alias resolves to `claude-sonnet-5`, no `{efforts.*}` substitution survives in dispatch.cjs (frontmatter `effort:` is the only lever the harness honors), and all 11 agents pin frontmatter effort, K240 verifies static-compress `.original.md` backups are excluded from the governing-rules inline set and surfaced in `paths_excluded` — the backup-inlining field bug that duplicated ~half the corpus per dispatch and cap-evicted live rules, K241 keeps legacy surfaces deleted — no `debug-knowledge-base` compat refs and no `context-monitor` hook (harness auto-compaction owns the context lifecycle), K242 asserts the dependency-legitimacy guardrail — registry-existence verification for new dependencies (slopsquatting defense) in `engineering-principles.md` + the researcher contract, K243 verifies the native `LSP` tool is declared on code-reviewer/debugger/programmer with LSP-first caller-enumeration guidance and a Grep fallback (runtime-wired callers the static graph misses; MCP-blind contract intact), K244 asserts the test-tampering defense — golden-rules never-weaken-tests rule + the verifier diffing test COUNTS against baseline so deletion-shaped gate gaming is visible, K245 enforces the graphify-helpers hot/cold split — hot fallback protocol preloaded, the ORCHESTRATOR-ONLY MCP surface + thresholds + parity tables lazy-loaded from references/ (the preloaded consumer is MCP-blind), K246 verifies the council skill runs forked (`context: fork`) with a fork-safe Stage-1 early return so ~15-25K tokens of advisor traffic stay out of the caller context, K247 asserts the session-handoff contract — `/devt:thread` user-discoverable, `create` distills the session rather than quizzing the user, thread files are uniquely slug-named (never overwritten across concurrent sessions), both handoff writers end with a copy-paste resume prompt, redaction rules cover captured context, and Goal/corrections carry verbatim anchors with a cut-priority order, K248 asserts the frozen acceptance-criteria contract — the verifier freezes acceptance-criteria.json on iteration 1 and later retries grade against it verbatim, K249 verifies the review calibration signals — per-finding confidence gate in code-review-guide (evidence-sufficiency bar with suppression telemetry, no-filtering contract preserved) + the `<evolution>` by-reference block in both the reviewer template and its compiled region, K250 asserts inline-rubric alias dedup so the shipped default rubric trio inlines under the 32KB cap instead of silently falling back to path-only, K251 verifies review-weight sees the working tree — uncommitted risk-surface files trip the veto, unknown flags error instead of silently no-oping, `--files` parses, K252 enforces anchor quality — exact-node resolution refuses prose words and dangling label-only orphans, and corpus-blind changed code files are surfaced (never prose files), K253 asserts manifest-hash freshness — mtime-matched scope files are FRESH regardless of commit lag, K254 verifies zero-result neighbor queries aggregate their drops per source file so the filtered-out caller set stays consumable, K255 verifies node-kind-gated auto-promotion — identifier-shaped dropped callers return as results on empty queries while prose/docstring nodes stay down, with a sub-reason taxonomy, K256 asserts the receipt-driven wiring — `manifest_fresh` staleness tier, per-lane `<memory_affects>` injection, rubric plugin-root path form, blast 60KB MCP default, bounded telemetry chain, god-node severity section, K257 verifies blast_radius shape-aware `max_bytes` — the ranked degrees array is retained and raw label arrays truncate first, K258 runs the external-source hygiene sweep — zero Claude attribution trailers in templates (platform settings own attribution), lessons/ in the weekly-report memory scan, fetched-content-is-data framing on the web-consuming skill, Artifact-tool delegation in report workflows, K259 verifies the warn-only cross-lane overlap warning — a file assigned to multiple lanes is surfaced with its owning lanes while registration still succeeds) and `scripts/test-locking.cjs` (20-worker concurrent state-write test) on every push. Version coherence, CHANGELOG coverage, and `workflow_type` registry coverage are enforced. Releases are tag-driven — push `vX.Y.Z` to fire `.github/workflows/release.yml` which extracts the matching CHANGELOG section into the GitHub release notes.
