...
GitHub Actions runs two suites on every push:

- **`scripts/smoke-test.sh`** — 1000+ assertions across every CLI subcommand, plus the deep drift-guard stack (`K94+`, 200+ gates) covering command stratification, parameter routing, stale-ref scans, `workflow_type` registry parity, size budgets, CLI surface contracts, telemetry validation, and substance-enforcement gates. Each gate's purpose is documented in the `## [X.Y.Z]` CHANGELOG entry that introduced it — see [CHANGELOG.md](CHANGELOG.md) for the per-gate detail.
- **`scripts/test-locking.cjs`** — 20-worker concurrent state-write test.

CI also enforces 3-way version coherence (`VERSION` ↔ `plugin.json` ↔ `marketplace.json`), CHANGELOG `[Unreleased]` coverage for code-surface commits, and `workflow_type` registry parity. Releases are tag-driven — pushing a `vX.Y.Z` tag fires `.github/workflows/release.yml`, which extracts the matching CHANGELOG section into the release notes.
